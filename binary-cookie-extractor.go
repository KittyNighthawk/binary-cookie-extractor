...
func extractCookiesFromPages(pages pages) {
  ...
  for i := 0; i < len(pages.pages); i++ {
    ...
    if *debug { fmt.Printf("[DEBUG] Number of cookies in page (%d): %d\n", i+1, pages.pages[i].numCookiesInPage) }
...
func extractPages(data []byte) pages {
  ...
  if *debug { fmt.Printf("[DEBUG] Number of pages: %d\n", pages.numPages) }
  pages.pageSizes = parseSizeOfPages(data, pages.numPages)
  pages.headerSize = pages.numPages * 4 + 8
  if *debug { fmt.Printf("[DEBUG] Size of header: %d bytes\n", pages.headerSize) }
  ...
  for i := 0; i < len(pages.pageSizes); i++ {
      ...
      if *debug { fmt.Printf("[DEBUG] Value of rawBytes in page %d: %v\n", i+1, page.rawBytes) }
  }
  return pages
}
...
func parseSizeOfPages(data []byte, pages uint64) []uint64 {
  ...
  for i := 0; i < int(pages); i++ {
    ...
    if *debug { fmt.Printf("[DEBUG] Size of page %d: %d bytes\n", i+1, pageSize) }
  }
  return result
}
